...
void main() {
  ...
  group('DailyActivityService - Dynamic Wake/Sleep Hours', () {
    test(
      'getEffectiveWakeHour returns profile default when no patterns',
      () async {
        final profile = createTestProfile(wakeUpHour: 6, sleepHour: 22);
        await profileRepo.saveProfile(profile);

        final wakeHour = await service.getEffectiveWakeHour(
          DateTime(2024, 1, 15),
        );
    ...
    test(
      'getEffectiveWakeHour returns learned weekend pattern for weekends',
      () async {
        final now = DateTime.now();
        final normalizedNow = DateTime(now.year, now.month, now.day);
    ...
    test(
      'getEffectiveWakeHour returns default 8 for weekend when no data',
      () async {
        // Find a Saturday
    ...
    test(
      'getEffectiveWakeHour returns default 7 for weekday when no data',
      () async {
        // Find a Monday
    ...
    test(
      'getEffectiveSleepHour returns profile default when no patterns',
      () async {
        final profile = createTestProfile(wakeUpHour: 6, sleepHour: 22);
        await profileRepo.saveProfile(profile);

        final sleepHour = await service.getEffectiveSleepHour(
          DateTime(2024, 1, 15),
        );
        expect(sleepHour, 22);
      },
