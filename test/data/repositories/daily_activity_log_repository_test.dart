...
void main() {
  ...
  group('DailyActivityLogRepository - CRUD', () {
    ...
    test('getOrCreateForDate returns existing log if exists', () async {
      ...
      final initial = DailyActivityLog.createForDate(date)
        ..tasksCompleted = 5;
  ...
  group('DailyActivityLogRepository - Pattern Aggregation', () {
    test('getActivityPatterns returns learned patterns from activity data', () async {
      final now = DateTime.now();
      final normalizedNow = DateTime(now.year, now.month, now.day);
      ...
      while (recentWeekend.weekday < 6) {
        recentWeekend = recentWeekend.subtract(Duration(days: 1));
      }
      
      final weekendLog = DailyActivityLog.createForDate(recentWeekend)
        ..firstActivityAt = DateTime(recentWeekend.year, recentWeekend.month, recentWeekend.day, 9, 0)
        ..lastActivityAt = DateTime(recentWeekend.year, recentWeekend.month, recentWeekend.day, 23, 0);
    ...
    test('getAverageWeekendWakeHour returns correct average', () async {
      ...
      final sunday = DailyActivityLog.createForDate(recentSunday)
        ..firstActivityAt = DateTime(recentSunday.year, recentSunday.month, recentSunday.day, 10, 0);
