...
class RuleBasedScheduler {
  ...
  Future<List<ScheduledTask>> scheduleForDate(DateTime date) async {
    print('🔄 Generating schedule for ${date.toIso8601String().split('T')[0]}');

    // 1. Get goals that should be scheduled for this day
    final goals = await _getActiveGoalsForDate(date);
    print('📋 Found ${goals.length} goals for this day');
    ...
    for (final goal in goals) {
      print(
        '\n🎯 Scheduling: ${goal.title} (${goal.targetDuration}min, priority: ${goal.priorityIndex})',
      );
      ...
      if (bestSlot != null) {
        ...
        print(
          '   ✅ Scheduled at ${task.scheduledStartTime.hour}:${task.scheduledStartTime.minute.toString().padLeft(2, '0')}',
        );
      } else {
  ...
  Future<List<ScheduledTask>> regenerateScheduleForDate(DateTime date) async {
    print(
      '🔄 Regenerating schedule for ${date.toIso8601String().split('T')[0]}',
    );
